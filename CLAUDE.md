...
- `const CARDS_EN = {章號: [{f, b}]}` — **全英文翻卡**，`CARDS_EN[n][i]` 位置對應 `CH[n].cards[i]`。
  載入時掛成 `c.en`；翻卡頁的「English only」核取方塊即切換。**張數對不上就整章退回中文。**
  熟練度是以卡片索引記錄，與語言無關，切換語言不影響已標記的進度。
- `const QUIZ_EN = {章號: [{q, opts, a, exp}]}` — **全英文題庫**，`QUIZ_EN[n][i]` 位置對應合併後的
  `CH[n].quiz[i]`。載入時掛成 `q.en`，測驗頁的「English only」核取方塊即切換語言。
  **掛載前會校驗題數、選項數與答案索引；任一項對不上就整章不掛英文、自動退回中文**，
  避免資料飄移造成答案錯位。改中文題（增刪或改選項順序）時要同步改 `QUIZ_EN`，否則該章英文會靜默停用。
...
- 24 章：**150 張翻卡**、**316 題測驗**（`CH` 內建 76 題 ＋ `EXTRA` 240 題，載入時合併，每章 12–14 題）、
  24 章講師版重點整理、12 週計畫。
- **測驗與翻卡都有中／英雙語**：316 題與 150 張卡全部備有全英文版，各自在該頁勾選
  「English only」即切換，UI 文字也一併換成英文。偏好分別存在 `localStorage` 的
  `quizEn` 與 `cardsEn`。
